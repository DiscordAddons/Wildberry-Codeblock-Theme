# What is a `codeblock theme`?
- A `codeblock theme` is a theme that changes the appearance of Discord's regular codeblocks. I made up this idea because I thought it would be cool for someone to make something called `codeblock themes`.

# Wildberry - Codeblock Theme [Discord's first, ever, codeblock theme?]
A `codeblock` theme that changes the appearance of Discord's codeblocks using [Wildberry](https://github.com/DagCord/Wildberry) colors. Planning on making another `codeblock theme` tomorrow.
```css
.markup-2BOw-j code {
    background: #350058;
    border: 1.5px solid #170027;
}
.hljs-name, .hljs-section, .hljs-selector-class, .hljs-selector-id, .hljs-title {
    color: #F40174;
    filter: brightness(100%);
}
.hljs-doctag, .hljs-literal, .hljs-meta .hljs-meta-string, .hljs-number, .hljs-regexp, .hljs-string {
    color: #00ffb7;
}
.hljs-attr, .hljs-attribute, .hljs-class .hljs-title, .hljs-template-variable, .hljs-type, .hljs-variable {
    color: #F40174;
    filter: brightness(200%);
}
```
